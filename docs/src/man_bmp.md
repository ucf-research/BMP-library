...
of each.
```julia
apply(htab, bmps; noclean=true)
apply(htab, bmps::BMP...; noclean=true)
minapply(htab, bmps; noclean=true)
minapply(htab, bmps::BMP...; noclean=true)
```
...
on any BMP obtained via `apply` and `clean1_rl` on any BMP obtained via
`minapply`. The algorithm used by `minapply` performs LTR cleaning "on the fly",
therefore there is no need for it later.
