function BMP(bdd::BDD)
    ...
    for i=1:n
        ...
        for (j,fid) in enumerate(funcs)
            nd = bdd.nodes[fid]
            lchild, hchild = nd.lchild, nd.hchild
            if nd.var_idx != i
            ...
            level_arr[i,2][j] = get!(level_ids, hchild, length(level_ids)+1)
        end
    ...
    R = [bdd.nodes[fid].hchild for fid in funcs]
...
function BDD(bmp::BMP)
    ...
    for i=n:-1:1
        for (j,p) in enumerate(zip(bmp.M[i,1].rows, bmp.M[i,2].rows))
            nodes[cnt+j] = BDDNode(i, p[1]+ref_cnt, p[2]+ref_cnt)
        end
